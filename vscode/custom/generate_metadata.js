...
import { error as _error, log } from 'console';
...
const getImages = _dir => {
  try {
    return readdirSync(_dir)
      .filter(item => {
        let extension = extname(`${_dir}${item}`);
...
const loadImgData = async _imgObject => {
  try {
    ...
    _error('Error loading image:', error);
  }
};
...
const saveMetadata = _loadedImageObject => {
  ...
  writeFileSync(`${buildDir}/${shortName}`, JSON.stringify(tempMetadata, null, 2));
  metadataList.push(tempMetadata);
};

const writeMetaData = _data => {
  writeFileSync(`${buildDir}/_metadata.json`, _data);
};

const startCreating = async () => {
  const images = getImages(inputDir);
  if (images == null) {
    log('Please generate collection first.');
    return;
  }
  ...
  await Promise.all(loadedImageObjects).then(loadedImageObjectArray => {
    loadedImageObjectArray.forEach(loadedImageObject => {
      ...
      log(`Created metadata for image: ${loadedImageObject.imgObject.filename}`);
    });
  });
